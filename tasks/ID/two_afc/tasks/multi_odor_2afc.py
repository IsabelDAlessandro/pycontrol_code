...
pc.v.reward_durations            = rwd_durations  # [left_ms, right_ms]
...
# =======================
pc.v.odors = [
    {'name': 'A', 'valve': 1, 'side': 'right'},
    {'name': 'B', 'valve': 2, 'side': 'left'},
    {'name': 'C', 'valve': 3, 'side': 'right'},
    {'name': 'D', 'valve': 4, 'side': 'left'},
]
...
pc.v.choice            = "none"      # 'left' or 'right'
...
def set_odor_valves():
    """
    Preset the upcoming odor by opening its manifold valve (#OLF:oN)
    while final_valve (TTL) stays closed. Matches your full task’s gating.
    """
    if pc.v.current_valve is not None:
        olf_open(pc.v.current_valve)
    pc.v.rewarded_side = pc.v.current_odor['side']
...
def select_next_trial():
    pc.v.current_odor  = pc.choice(pc.v.odors)         
...
events = [
    ...
    "center_poke_held",
    "set_odor_valves_for_trial", "choice_window_over", "svf_recheck"
]
...
def deliver_odor(event):
    if event == "entry":
        ...
        disable_odor_valves()  # closes the specific chosen valve (#OLF:cN)
...
def inter_trial_interval(event):
    if event == "entry":
        ...
        pc.print_variables([
            ...
            "current_odor", "rewarded_side", "choice", "outcome"
        ])
