...
from hardware_definition import right_port, left_port, center_port, final_valve, odor_A, odor_B, thermistor_sync, rwd_durations


# Rwd sizing
# For rwd durn multplier of 1, 1 mL ~ 125 rewards. should be 200???
# For rwd durn multiplier of 0.75, ~ 225 rewards.
pc.v.reward_duration_multiplier = 1 #0.75 #change this to 1 **** i dont like the random multiplier thing
...
pc.v.n_allowed_rwds_per_block = 3 #change depending on definition above!! *** ? or is this just initializing and can set to 0
pc.v.n_rewards_in_block = 0  # per-block counter

# Re-define these functions here to produce odor behavior
def set_odor_valves():
    #make sure clean slate each time
    odor_A.off()
    odor_B.off()
...
pc.v.timeout_duration = 1.5 * pc.second  # timeout for wrong trials (in addition to ITI)
